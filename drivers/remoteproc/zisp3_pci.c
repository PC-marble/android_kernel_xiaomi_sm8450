...
static struct mem_segment zispv3_ipc_segment = { 0x80880000, 0x80880000,
						 64 * 1024 };
#endif
				  ...
				  void __iomem *regs)
{
	...
	zisp_rproc->regmap = regmap_init_mmio(&pdev->dev, regs, &reg_config);
...
static int zisp3_rproc_start(struct rproc *rproc)
{
	...
	regmap_write(zisp_rproc->regmap, ZISPV3_AIO1, rproc->bootaddr >> 16);
...
static int zisp3_mcu_init(struct zisp_rproc *zisp_rproc)
{
	...
	if (iommu_map(domain, zisp_rproc->ipc_dma_addr, zisp_rproc->ipc_membase,
		      zisp_rproc->ipc_size,
