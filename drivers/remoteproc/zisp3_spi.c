...
static int zisp3_rproc_start(struct rproc *rproc)
{
	...
	zispv3_spi_write_reg(zisp_rproc->spi,
			     zisp_rproc->reg_base + ZISPV3_MCU_CLK_EN,
			     0xffffc001);
	zispv3_spi_write_reg(zisp_rproc->spi,
			     zisp_rproc->reg_base + ZISPV3_MCU_CLK_RST,
			     0xffffc001);
...
static void *zisp3_mcu_rproc_da_to_va(struct rproc *rproc, u64 da, size_t len,
				      bool *is_iomem)
{
	return (void *)da;
}
			       ...
			       struct resource **_iores)
{
	...
	for (i = 0; i < ZISPV3_MEM_NR; ++i) {
		...
		mem->virt_addr =
			devm_ioremap_wc(&pdev->dev, mem->phys_addr, mem->size);
...
static int zisp3_mcu_parse_irq(struct platform_device *pdev, int irq)
{
	struct zisp_rproc *zisp_rproc;
...
static struct elf32_shdr *find_table(struct device *dev, struct elf32_hdr *ehdr,
				     size_t fw_size)
{
...
static int zisp3_elf_load_rsc_table(struct rproc *rproc,
...
static int zisp3_elf_load_segments(struct rproc *rproc,
				   const struct firmware *fw)
{
	struct device *dev = &rproc->dev;
	struct elf32_hdr *ehdr;
